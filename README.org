...
=twizzle= doesn't have a particular notion of time: it just uses timeline values which can be milliseconds (for realtime animation), frames (for rendering), floats, or anything else numerical.
...
#+BEGIN_SRC clojure
  (tw/automate state :my-param 200 10 1.0)
#+END_SRC
